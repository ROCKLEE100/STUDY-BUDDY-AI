pipeline {
    agent any
        environment {
            DOCKER_HUB_REPO = "aditya1243/studdybuddy"
            ...
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'git-token', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
                        sh '''
                        git config user.name "ROCKLEE100"
                        git config user.email "adityarathod7799@gmail.com"
            ...
            steps {
                script {
                    kubeconfig(credentialsId: 'kubeconfig', serverUrl: 'https://192.168.49.2:8443') {
                        sh '''
                        argocd login 136.112.54.129:31704 --username admin --password $(kubectl get secret -n argocd argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d) --insecure
